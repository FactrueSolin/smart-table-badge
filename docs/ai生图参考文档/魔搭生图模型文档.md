网址https://www.modelscope.cn/docs/model-service/API-Inference/intro
...
while True:
    ...
    time.sleep(5)
Base64 图片输入支持#
对于图像编辑模型，输入的图片建议使用线上托管的图像URL地址。如果需要使用本地文件，可以使用上方的image_to_data_url函数对本地图片进行Base64编码，并将编码后的数据作为image_url传递给模型。一个典型的调用方式是：
